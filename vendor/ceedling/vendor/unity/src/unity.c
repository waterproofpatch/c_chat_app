                              ...
                              const UNITY_FLAGS_T         flags)
{
    ...
    while ((elements > 0) && elements--)
    {
        ...
        if (expect_val != actual_val)
        {
            ...
            { /* For UINT, remove sign extension (padding 1's) from signed type
                 casts above */
                UNITY_INT mask = 1;
                                ...
                                const UNITY_FLAGS_T   flags)
{
    ...
    while (elements--)
    {
        if (!UnityFloatsWithin(*ptr_expected * UNITY_FLOAT_PRECISION,
                               *ptr_expected,
                               *ptr_actual))
        {
            UnityTestResultsFailBegin(lineNumber);
    ...
    if ((style & UNITY_DISPLAY_RANGE_INT) == UNITY_DISPLAY_RANGE_INT)
    {
        if (actual > expected)
            Unity.CurrentTestFailed =
                (UNITY_UINT)((UNITY_UINT)(actual - expected) > delta);
        else
            Unity.CurrentTestFailed =
                (UNITY_UINT)((UNITY_UINT)(expected - actual) > delta);
    }
    else
    {
...
int IsStringInBiggerString(const char *longstring, const char *shortstring)
{
    ...
    while (*lptr)
    {
        ...
        /* Otherwise we start in the long pointer 1 character further and try
         * again */
        lptr = lnext;
        sptr = shortstring;
    }
