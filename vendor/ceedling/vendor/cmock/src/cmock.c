...
const char *CMockStringOutOfMemory =
    "CMock has run out of memory. Please allocate more.";
...
#else
static CMOCK_MEM_INDEX_TYPE
    CMock_Guts_BufferArray[(CMOCK_MEM_SIZE + CMOCK_MEM_INDEX_SIZE - 1) /
                           CMOCK_MEM_INDEX_SIZE];
#define CMock_Guts_Buffer ((unsigned char *)CMock_Guts_BufferArray)
static CMOCK_MEM_INDEX_TYPE CMock_Guts_BufferSize =
    CMOCK_MEM_SIZE + CMOCK_MEM_ALIGN_SIZE;
static CMOCK_MEM_INDEX_TYPE CMock_Guts_FreePtr;
#endif
...
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemNew(CMOCK_MEM_INDEX_TYPE size)
{
    ...
    // verify arguments valid (we must be allocating space for at least 1 byte,
    // and the existing chain must be in memory somewhere)
    if (size < 1)
    ...
    if ((CMock_Guts_BufferSize - CMock_Guts_FreePtr) < size)
    {
        ...
        return CMOCK_GUTS_NONE;   // nothing we can do; our static buffer is out
                                  // of memory
#else
        // our dynamic buffer does not have enough room; request more via
        // realloc()
        CMOCK_MEM_INDEX_TYPE new_buffersize =
            CMock_Guts_BufferSize + CMOCK_MEM_SIZE + size;
        unsigned char *new_buffer =
            realloc(CMock_Guts_Buffer, (size_t)new_buffersize);
        if (new_buffer == NULL)
    ...
    // determine where we're putting this new block, and init its pointer to be
    // the end of the line
    index = CMock_Guts_FreePtr + CMOCK_MEM_INDEX_SIZE;
    *(CMOCK_MEM_INDEX_TYPE *)(&CMock_Guts_Buffer[CMock_Guts_FreePtr]) =
        CMOCK_GUTS_NONE;
...
// CMock_Guts_MemChain
//-------------------------------------------------------
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemChain(CMOCK_MEM_INDEX_TYPE root_index,
                                         CMOCK_MEM_INDEX_TYPE obj_index)
{
    CMOCK_MEM_INDEX_TYPE index;
    void *               root;
    void *               obj;
    ...
    {
        // if there is no root currently, we return this object as the root of
        // the chain
        return obj_index;
    }
    else
    {
        ...
        if ((root_index < CMOCK_MEM_ALIGN_SIZE) ||
            (root_index >= CMock_Guts_FreePtr))
        {
            return CMOCK_GUTS_NONE;
        }
        if ((obj_index < CMOCK_MEM_ALIGN_SIZE) ||
            (obj_index >= CMock_Guts_FreePtr))
        {
            return CMOCK_GUTS_NONE;
        }
        ...
        do
        {
            index = *(CMOCK_MEM_INDEX_TYPE *)((CMOCK_MEM_PTR_AS_INT)next -
                                              CMOCK_MEM_INDEX_SIZE);
        ...
        *(CMOCK_MEM_INDEX_TYPE *)((CMOCK_MEM_PTR_AS_INT)next -
                                  CMOCK_MEM_INDEX_SIZE) =
            (CMOCK_MEM_INDEX_TYPE)((CMOCK_MEM_PTR_AS_INT)obj -
                                   (CMOCK_MEM_PTR_AS_INT)CMock_Guts_Buffer);
        return root_index;
    }
}
...
// CMock_Guts_MemNext
//-------------------------------------------------------
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemNext(
    CMOCK_MEM_INDEX_TYPE previous_item_index)
{
    CMOCK_MEM_INDEX_TYPE index;
    void *               previous_item;

    // There is nothing "next" if the pointer isn't from our buffer
    if ((previous_item_index < CMOCK_MEM_ALIGN_SIZE) ||
        (previous_item_index >= CMock_Guts_FreePtr))
        return CMOCK_GUTS_NONE;
    previous_item = (void *)(&CMock_Guts_Buffer[previous_item_index]);

    // if the pointer is good, then use it to look up the next index
    //(we know the first element always goes in zero, so NEXT must always be >
    // 1)
    index = *(CMOCK_MEM_INDEX_TYPE *)((CMOCK_MEM_PTR_AS_INT)previous_item -
                                      CMOCK_MEM_INDEX_SIZE);
...
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemEndOfChain(CMOCK_MEM_INDEX_TYPE root_index)
{
    ...
    for (next_index = root_index; next_index != CMOCK_GUTS_NONE;
